...
async function checkWithdrawDestination(method) {
  try {
    if (method === "JPY") {
      // 銀行口座チェック
      const res = await fetch(CONFIG.API_BASE_URL + "/user/bank-account", {
        credentials: "include",
      });
    ...
    if (method === "CRYPTO") {
      // 暗号資産アドレスチェック
      const res = await fetch(CONFIG.API_BASE_URL + "/user/crypto", {
        credentials: "include",
      });
