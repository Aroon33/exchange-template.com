...
async function sendDepositAddress(id) {
  if (!confirm("入金アドレスを送信しますか？")) return;
  await apiPost("/deposit/assign-crypto", { id });
  showToast("入金アドレスを送信しました");
  loadDeposits();
}
...
function openDepositModal(t) {
  document.getElementById("modal-body").innerHTML = `
    <div>ID：${t.id}</div>
    <div>ユーザー：${t.user?.name ?? "-"}</div>
    <div>通貨：${t.currency}</div>
    <div>金額：${t.cryptoAmount ?? t.amount}</div>
    <div>状態：${t.status}</div>
  ...
  document.getElementById("modal-footer").innerHTML = `
    ${t.status==="PENDING" && t.method==="CRYPTO"
      ? `<button class="btn" onclick="sendDepositAddress(${t.id})">アドレス送信</button>` : ""}
    ${t.status==="CONFIRMING"
      ? `<button class="btn" onclick="approveDeposit(${t.id})">着金確認</button>` : ""}
    <button class="btn" onclick="openChat(${t.user?.id})">チャット</button>
  `;
...
function renderTable(list) {
  ...
  if (!list.length) {
    tbody.innerHTML = `<tr><td colspan="100%" style="text-align:center;">申請なし</td></tr>`;
    return;
  }
...
<tr class="deposit-row"
  ...
  <td class="actions pc-only">
    ${t.status==="PENDING" && t.method==="CRYPTO"
      ? `<button onclick="event.stopPropagation();sendDepositAddress(${t.id})">送信</button>` : ""}
    ${t.status==="CONFIRMING"
      ? `<button onclick="event.stopPropagation();approveDeposit(${t.id})">確認</button>` : ""}
    <button onclick="event.stopPropagation();openChat(${t.user?.id})">💬</button>
  </td>
</tr>
`);
  });
